# 🔪 AUTO_PILOT SURGICAL v3 — صفر Hangs · صفر Loops

> **التطوّر الثالث:** بعد PRO، اكتشفنا الـ hang الحقيقي = `str_replace` retry loops على ملفات 2.2MB.
> **الحل:** Script-First + Single Tool Per Response + Mandatory Pre-Verify + Hang Detection.
> **اقرأ هذا الملف فقط (240 سطر) ولا شي ثاني — الـ phase files مرجعية، تُقرأ بـ targeted slices.**

---

## 📋 برومت الاستخدام (انسخه كاملاً)

```
أنت AUTO_PILOT SURGICAL — Pack v3 DEVOTIO.
...
═══ SCRIPT TEMPLATE (للـ Strategy B) ═══

scripts/_phase<P>_step<N>.mjs:
...
// Edit 1
s = s.replace(/<oldPattern1>/, '<newReplacement1>');
// Edit 2
s = s.replace(/<oldPattern2>/, '<newReplacement2>');
// (more as needed — but ≤5 per script)

writeFileSync(F, s);
console.log('✓ Applied N edits');
```

تنفيذ: execute_bash "node scripts/_phase<P>_step<N>.mjs"
حذف بعد النجاح: execute_bash "rm scripts/_phase<P>_step<N>.mjs"

═══ READING BUDGET (per session) ═══

| Source | Max | Method |
|---|---|---|
| state/PROGRESS.json | full | read |
| Worker index (step=1 only) | 250 lines | read full |
| Phase file (per step) | 120 lines × ≤6 steps = 720 | offset+limit per step |
| platform/* (للـ context) | 50 lines × ≤8 reads = 400 | grep + offset+limit |
...
## 💡 لماذا SURGICAL يحل الـ Hang

| المشكلة | الحل |
|---|---|
| `str_replace` retry loops | **3-fail rule** → switch to script |
| Edit على ملف 2.2MB يأخذ دقيقة | **Single tool per response** — تشوف الـ progress |
| AI يحاول 5 edits متتالية | **Strategy B** — script واحد atomic |
| ما تعرف وين توقّف | **Heartbeat output** بعد كل tool call |
| Session ينتهي بدون شي | **Tool budget 25/session** — break مُبكر |
| Phase file يُقرأ كاملاً | **Step-by-step reading** بـ offset+limit |

## 🎯 الفروقات بين النسخ الأربع

| | LITE | PRO | **SURGICAL** ⭐ | الأصلي |
|---|:---:|:---:|:---:|:---:|
| طول البرومت | 80 | 220 | **240** | 300 |
| Single tool per response | ❌ | ⚠️ implicit | ✅ **explicit** | ❌ |
| Hang detection | ❌ | ❌ | ✅ **3-fail rule** | ❌ |
| Script-first for huge files | ❌ | ❌ | ✅ **Strategy B** | ❌ |
| Tool budget tracking | ❌ | ⚠️ | ✅ **strict 25/session** | ❌ |
| Heartbeat output | ❌ | ⚠️ | ✅ **per tool call** | ❌ |
| Pre-verify mandatory | ❌ | ⚠️ | ✅ **before every str_replace** | ❌ |
| **يقاوم hang** | ❌ | ⚠️ | ✅ **نعم** | ❌ |
| **متوسط النجاح** | 60% | 85% | **92%+** | 70% |

<function_calls>
<invoke name="run_command">
<parameter name="command">git -C /projects/sandbox/Upgrade add prompts/v3/AUTO_PILOT_SURGICAL.md && git -C /projects/sandbox/Upgrade commit -m "add AUTO_PILOT_SURGICAL — anti-hang version with single-tool-per-response, scripts for huge files, 3-fail rule, tool budget"