...
/* ════════════════════════════════════════════════════════════════════════
   ...
   ════════════════════════════════════════════════════════════════════════
   27th top-level Upg.* namespace.
   Phase 1 introduces Entry Ritual; Phases 2-6 will EXTEND this IIFE
   (additive freeze-replace) with reading halo, threshold transitions,
   inkpot feedback, time-of-day, and aura deepening.
   ════════════════════════════════════════════════════════════════════════ */
(function (window, document) {
  'use strict';
  ...
  /* 7 hand-curated Arabic poetry lines — read-only canon.
     Do not invent more without spec approval. */
  const POETRY_LINES = Object.freeze([
    'بسم اللحظة، نَبدأ.',
    'الحرف يَستقبل اليد.',
    'اليوم — مرة أخرى.',
  ...
  const isDisabled = (rituId) => readDisabledList().includes(rituId);

  const setDisabled = (rituId, on) => {
    try {
      const set = new Set(readDisabledList());
  ...
  const dismissEntry = () => {
    ...
    if (entryCleanup) { entryCleanup(); entryCleanup = null; }
  ...
  const startEntry = (force) => {
    ...
    /* Mark today's entry — even on force, we set so a second force in same
       session won't auto-fire on next reload. */
    try { localStorage.setItem(STORAGE_KEY_LAST_ENTRY, String(Date.now())); }
    catch (_) { /* ignore */ }
    ...
    if (poetryEl) {
      ...
      poetryEl.textContent = POETRY_LINES[idx];
    }

    /* Reduced-motion marker (CSS handles instant display) */
    const rm = isReducedMotion();
    if (rm) portal.setAttribute('data-rit-rm', 'reduce');
    ...
    /* Auto-dismiss timing */
    const dismissDelay = rm ? 800 : 4000;
    entryTimer = setTimeout(dismissEntry, dismissDelay);

    /* Skip handlers */
    const skipBtn = portal.querySelector('[data-rit-skip]');
    const onSkip = (e) => { if (e) e.preventDefault(); dismissEntry(); };
    const onKeydown = (e) => {
    ...
    const onPortalClick = (e) => {
      ...
      if (!t) return;
      if (t === portal ||
          t.classList.contains('rit-entry-veil') ||
          t.classList.contains('rit-entry-stage')) {
        dismissEntry();
      }
    };
  ...
  const start = (rituId, opts) => {
    ...
    if (rituId === 'entry') return startEntry(force);
    if (window.console && console.warn) {
  ...
  const disable = (rituId) => { setDisabled(rituId, true);  return true; };
  const enable  = (rituId) => { setDisabled(rituId, false); return true; };

  const status = () => {
    const lastEntry = localStorage.getItem(STORAGE_KEY_LAST_ENTRY);
    const ts = lastEntry ? parseInt(lastEntry, 10) : null;
    return {
      entry: {
        last_run:   (ts && Number.isFinite(ts)) ? new Date(ts).toISOString() : null,
        ran_today:  isToday(lastEntry),
        disabled:   isDisabled('entry'),
        active_now: entryActive
      }
    };
  };

  const list = () => Object.freeze(['entry']);

  /* ── Auto-trigger entry on first DOMContentLoaded of the day ─────── */

  const autoFire = () => {
    /* Small delay to let layout/fonts settle to avoid wordmark FOIT flash */
    setTimeout(() => { try { startEntry(false); } catch (_) {} }, 100);
  };
  ...
  window.Upg.ritual = Object.freeze({
    start, stop, disable, enable, status, list,
    POETRY_LINES
  });
})(window, document);
/* End RITUAL UI v3 / Worker 22 / Phase 1 — Entry Ritual ──────────────── */



/* ════════════════════════════════════════════════════════════════════════
   RITUAL UI v3 — Reading Halo Logic (Worker 22 / Phase 2)
   EXTENDS Upg.ritual (from P1) with halo methods.
   No new top-level API — Upg.ritual count stays at 1 namespace (27 total).
   Sacred preserved: P1 surface (start/stop/disable/enable/status/POETRY_LINES)
   remains intact; we re-freeze with extended members.
   ════════════════════════════════════════════════════════════════════════ */
(function (window, document) {
  'use strict';
  if (!window.Upg || !window.Upg.ritual) return;  // requires P1 IIFE first
  ...
  // Pick a sensible default target if none specified.
  const defaultTarget = () => {
    const visible = (sel) => {
      const el = document.querySelector(sel);
      if (!el) return null;
      // crude visibility: not display:none + has size
      const style = window.getComputedStyle(el);
      if (style.display === 'none' || style.visibility === 'hidden') return null;
      return el;
    };
    return visible('section.page:not([hidden]) .page-body') ||
           visible('section.page:not([hidden]) main') ||
           visible('section.page:not([hidden]) article') ||
           visible('section.page:not([hidden])') ||
           visible('main') ||
           document.body;
  };
  ...
  const onOutsideClick = (e) => {
    ...
    if (t.closest('.rit-halo-toggle')) return;
  ...
  function enterHalo(targetOrSelector) {
    ...
    // Exit existing different halo first
    ...
    // capture phase so chrome clicks dismiss before reaching disabled handlers
  ...
  function exitHalo() {
    ...
    if (exitButton && exitButton.parentNode) {
      try { exitButton.parentNode.removeChild(exitButton); } catch (e) {}
    }
    exitButton = null;
    activeTarget = null;
  ...
  function lastHaloTarget() {
    try { return localStorage.getItem(STORAGE_KEY_LAST_HALO); }
    catch (e) { return null; }
  }
  ...
  // EXTEND Upg.ritual without breaking P1 frozen members.
  // Strategy: re-build a frozen object inheriting all P1 keys + new halo keys.
  const prev = window.Upg.ritual;
  const merged = {
    // P1 surface (verbatim)
    start:   prev.start,
    stop:    prev.stop,
    ...
    enable:  prev.enable,
    status:  prev.status,
    POETRY_LINES: prev.POETRY_LINES,
    // P2 additions
    enterHalo,
    exitHalo,
    toggleHalo,
  ...
  window.Upg.ritual = Object.freeze(merged);
})(window, document);
/* End RITUAL UI v3 / Worker 22 / Phase 2 — Reading Halo Logic ──────── */


/* ════════════════════════════════════════════════════════════════════════
   RITUAL UI v3 — Threshold Routing (Worker 22 / Phase 3)
   EXTENDs Upg.transition.run with ritual option + adds threshold helpers
   to Upg.ritual. No new top-level Upg.* namespace (still 27).
   ════════════════════════════════════════════════════════════════════════ */
(function (window, document) {
  'use strict';
  ...
  // Get transition variant for a page personality
  var getTransitionForPage = function (pageId) {
    var stripped = (pageId || '').replace(/^page-/, '');
    return PERSONALITY_TRANSITIONS[stripped] || 'fade';
  };

  // Set body data attribute (used by CSS @supports view-transitions)
  ...
  // ─── EXTEND Upg.transition.run ───────────────────────────────────────
  var originalRun = window.Upg.transition.run;

  var extendedRun = function (name, opts) {
    opts = opts || {};
    var variant = opts.ritual ||
                  (opts.toPageId ? getTransitionForPage(opts.toPageId) : null) ||
                  'fade';

    // In reduced-motion, always use fade (instant)
    if (isReducedMotion()) variant = 'fade';

    setActiveTransition(variant);

    // Use View Transitions API if available and variant is non-fade
    if (document.startViewTransition && variant !== 'fade' && !isReducedMotion()) {
      var vt = document.startViewTransition(function () {
        if (typeof originalRun === 'function') originalRun(name, opts);
      });
      return vt;
    }

    // CSS class-based fallback animation
    if (variant !== 'fade' && !isReducedMotion()) {
      var pages = document.querySelectorAll('section.page');
      var oldEl = null;
      var newEl = opts.toPageId ? document.getElementById(opts.toPageId) : null;

      for (var i = 0; i < pages.length; i++) {
        if (!pages[i].hasAttribute('hidden') && pages[i] !== newEl) {
          oldEl = pages[i];
          break;
        }
      }
    ...
    // Call original run (preserves all W14 behavior)
    if (typeof originalRun === 'function') return originalRun(name, opts);
  };

  // Replace run on the frozen object by creating new extended object
  var transitionExtended = Object.assign({}, window.Upg.transition, { run: extendedRun });
  window.Upg.transition = Object.freeze(transitionExtended);

  // ─── EXTEND Upg.ritual with threshold helpers ────────────────────────

  var ritualCurrent = window.Upg.ritual;
  var ritualExtended = Object.assign({}, ritualCurrent, {
    setTransition: function (variant, pagePersonality) {
      if (pagePersonality) {
    ...
    getTransition: function (pagePersonality) {
      return PERSONALITY_TRANSITIONS[(pagePersonality || '').replace(/^page-/, '')] || 'fade';
    },
    listTransitions: function () { return VARIANTS.slice(); },
    transitionMap: function () {
      var copy = {};
      for (var k in PERSONALITY_TRANSITIONS) {
        if (PERSONALITY_TRANSITIONS.hasOwnProperty(k)) copy[k] = PERSONALITY_TRANSITIONS[k];
      }
      return copy;
    }
  });
  ...
  // ─── Auto-route on upg:nav:change ────────────────────────────────────
  document.addEventListener('upg:nav:change', function (e) {
    var pageId = (e.detail && e.detail.pageId) || '';
    var variant = getTransitionForPage(pageId);
    setActiveTransition(variant);
  });
